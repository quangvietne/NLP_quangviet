...
object Lab17_NLPPipeline {
  def main(args: Array[String]): Unit = {
    ...
    val logWriter = new PrintWriter(new FileWriter(log_path, true)) // true = append mode
    try {
      ...
      if (1000 < actualVocabSize) {
        logWriter.println(s"Note: numFeatures (1000) is smaller than actual vocabulary size ($actualVocabSize). Hash collisions are expected.")
      }
    ...
    val resultWriter = new PrintWriter(new FileWriter(result_path, true)) // true = append mode
    try {
